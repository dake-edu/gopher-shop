...
Редакция фрагмента: **0.21.0-sample**. Примеры проверяются на Go 1.27.1. Бесплатный фрагмент позволяет попробовать методику и код перед покупкой. Главы 13–42 и полные платные файлы в этот репозиторий и его выпуски не входят.
...
В редакции 0.21 главы 3–12 содержат 18 небольших запускаемых опытов: от переменных до HTTP и шаблонов. Ожидаемый вывод проверяется автоматически. В полной книге дополнительно переработаны переходы к JSON, SQL, интерфейсам, конкурентности и обобщениям. Техническая проверка не заменяет самостоятельное пробное прохождение начинающим читателем.
