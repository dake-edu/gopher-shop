...
## Новая книга: Go от первой строки до книжного магазина

В [book/](book/README.md) создаётся русскоязычная книга для shanraq.org с тремя уровнями: Junior, Middle и Senior. Сквозной проект — магазин электронных книг, первым товаром которого станет сама книга.

Уже написаны вступление и главы 1–9. Контрольные точки проверяются на Go 1.27.1; PDF, EPUB и офлайн-HTML собираются из одного текста. Это рабочая редакция, а не законченный магазин. [Программа](book/editorial/blueprint.md) и [состояние работы](book/CONTINUE.md).

```sh
python3 book/tools/check.py
```

Ниже сохранено описание предыдущей версии курса. Её примеры в `lessons/`, документация в `docs/` и корневое API будут сопоставляться с новой книгой по мере переработки.
